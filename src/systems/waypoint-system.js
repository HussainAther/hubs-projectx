    ...
    return function onInteract() {
      ...
      this.tryToOccupy(waypointComponent).then(didOccupy => {
        if (didOccupy) {
          ...
          if (waypointComponent.el.object3D.name.startsWith("PX_Seat_")) {
            this.scene.emit("px-seat-selected", { waypoint: waypointComponent });
          }
    ...
    const tickTemplateEl = (elementFromTemplate, waypointComponent) => {
      if (
        ...
        elementFromTemplate.object3D.visible =
          this.scene.is("frozen") || waypointComponent.el.object3D.name.startsWith("PX_Seat_");
